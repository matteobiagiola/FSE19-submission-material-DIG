...
public class SearchStatistics implements Listener<ClientStateInformation>{
	...
	public void currentIndividual(String rmiClientIdentifier, Chromosome individual) {
		...
		/*if(individual instanceof TestSuiteChromosome)
		{
			...
			TestSuiteChromosome individualSuite = new TestSuiteChromosome();
			TestChromosome individualChromosome = (TestChromosome) individual;
			individualSuite.addTest(individualChromosome.getTestCase());
			individualSuite.setTestChromosome(0, (TestChromosome) individual);

			bestIndividual.put(rmiClientIdentifier, (TestSuiteChromosome) individualSuite);
	        for(ChromosomeOutputVariableFactory<?> v : variableFactories.values()) {
	            setOutputVariable(v.getVariable((TestSuiteChromosome) individualSuite));
	        }
			for(SequenceOutputVariableFactory<?> v : sequenceOutputVariableFactories.values()) {
				v.update((TestSuiteChromosome) individualSuite);
			}
		}*/
	}
