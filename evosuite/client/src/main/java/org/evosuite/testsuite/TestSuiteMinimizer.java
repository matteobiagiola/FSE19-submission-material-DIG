    ...
    private void minimizeSeleniumTests(TestSuiteChromosome suite){
        ...
        for(TestChromosome test: suite.getTestChromosomes()){
            ...
            try {
                ...
                ExecutionResult executionResult = TestCaseExecutor.runTest(copy.getTestCase());
            ...
            copy = pageObjectTestCaseMinimizer.minimizeIndividual(copy);
            copy.setChanged(true);

            // make sure that the minimized test case does not throw exceptions
            try {
                logger.debug("Executing minimized test");
                ExecutionResult executionResult = TestCaseExecutor.runTest(copy.getTestCase());
                Collection<Throwable> thrownExceptions = executionResult.getAllThrownExceptions();
                String exceptions = thrownExceptions.stream()
                        .map(throwable -> "Message: " + throwable.getMessage() + ". Cause: " + throwable.getCause() + "."
...
//							return toPrint + "\n" + stackTrace;
    ...
    private void minimizeTests(TestSuiteChromosome suite) {
        ...
        for (TestChromosome test : suite.getTestChromosomes()) {
            test.setChanged(true); // implies test.clearCachedResults();
            test.getTestCase().clearCoveredGoals(); // clear goals covered by test
        }
        ...
        for (TestFitnessFunction goal : goals) {
            ...
            for (TestChromosome test : suite.getTestChromosomes()) {
                if (goal.isCovered(test)) {
                    logger.info("minimizeTests - test case with Id " + test.getTestCase().getID() + " covers goal " + goal
                            + ". Goal " + goal + " is covered by this test? " + test.getTestCase().isGoalCovered(goal));
...
//                // TODO: Need proper list of covered goals
        ...
        suite.tests.clear();
        for (TestCase test : minimizedSuite.getTestCases()) {
            suite.addTest(test); // clear cached results
        }

        if (Properties.MINIMIZE_SECOND_PASS) {
            // going to re-execute all the tests in the suite
