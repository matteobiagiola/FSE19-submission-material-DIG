...
public class InstrumentingClassLoader extends ClassLoader {
	...
	public Class<?> loadClassFromFile(String fullyQualifiedTargetClass, String fileName) throws ClassNotFoundException {
		...
		try(InputStream is = new FileInputStream(new File(fileName))) {
			...
			logger.debug("Loaded class " + fullyQualifiedTargetClass + " directly from "+fileName);
			return result;
		} catch (Throwable t) {
			logger.debug("Error while loading class " + fullyQualifiedTargetClass + " : " + t);
	...
	public Class<?> loadClass(String name) throws ClassNotFoundException {
		...
		if (result != null) {
			...
			logger.debug("Seeing class for first time: " + name);
	...
	private Class<?> instrumentClass(String fullyQualifiedTargetClass)throws ClassNotFoundException  {
		...
		try {
			...
			logger.debug("Loaded class: " + fullyQualifiedTargetClass);
			return result;
		} catch (Throwable t) {
			logger.debug("Error while loading class: "+t);
			throw new ClassNotFoundException(t.getMessage(), t);
		} finally {
			if(is != null)
				try {
	...
	private void createPackageDefinition(String className){
		...
		if (i != -1) {
		    ...
		    if(pkg==null){
		    		...
		    		logger.debug("Defined package (3): "+getPackage(pkgname)+", "+getPackage(pkgname).hashCode());
		    }
	    }
	}
