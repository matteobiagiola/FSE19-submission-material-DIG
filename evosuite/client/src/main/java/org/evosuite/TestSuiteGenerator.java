...
public class TestSuiteGenerator {
	...
	public TestGenerationResult generateTestSuite() {
		...
		if(Properties.CUT_EXCEPTIONS && !Properties.MINIMIZE){
			throw new IllegalStateException("generateTestSuite: when 'cut_exceptions = true' also 'minimize' must be true");
		}
...
//			test.addStatement(loadClassStmt);
	...
	protected void postProcessTests(TestSuiteChromosome testSuite) {
		...
		/*if(goals.size() > 1)
		{
			TestFitnessFunction secondLastGoal = goals.get(goals.size() - 2);
			goals.clear();
			goals.add(secondLastGoal);
		}*/
...
//		}
	...
	private TestGenerationStrategy getTestGenerationStrategy() {
		...
		case CUSTOM_RANDOM:
			logger.debug("%%%%CUSTOM RANDOM STRATEGY");
			return new CustomRandomTestStrategy();
		case RANDOM_FIXED:
		...
		case MOSUITE:
			logger.debug("%%%%MOSA STRATEGY");
			return new MOSuiteStrategy();
		case DSE:
			return new DSEStrategy();
		case ADAPTIVE_RANDOM:
		{
			logger.debug("%%%%ADAPTIVE STRATEGY");
			return new AdaptiveRandomTestStrategy();
		}
		default:
