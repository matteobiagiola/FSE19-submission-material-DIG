...
public class Properties {
	...
	public enum Algorithm {
		STANDARDGA, MONOTONICGA, ONEPLUSONEEA, STEADYSTATEGA, RANDOM, NSGAII, MOSA, ADAPTIVE, CUSTOM_RANDOM
	}
	...
	public enum Strategy {
	    ONEBRANCH, EVOSUITE, RANDOM, RANDOM_FIXED, ENTBUG, REGRESSION, MOSUITE, DSE, ADAPTIVE_RANDOM, CUSTOM_RANDOM
	}
	...
	// ---------------------------------------------------------------
	// Selenium Plug-in flag
	
	@Parameter(key = "genotype_length", group = "Selenium", description = "Number of parameters to encode into the genotype of an individual")
	public static int GENOTYPE_LENGTH = 0;
	
	@Parameter(key = "chromosome_parts", group = "Selenium", description = "Number of chromosome within a chromosome for performance reason")
	public static int CHROMOSOME_PARTS = 1;
	
	@Parameter(key = "selenium_addon", group = "Selenium", description = "Activate or deactivate evosuite modifications (individual test strategy) for selenium purpose")
	public static boolean SELENIUM = false;
	
	@Parameter(key = "selenium_whole", group = "Selenium", description = "Activate or deactivate evosuite modifications (other strategies) for selenium purpose")
	public static boolean SELENIUM_WHOLE = true;
	
	@Parameter(key = "chromedriver_address", group = "Selenium", description = "Address of the chromedriver (the only supported at the moment) tool in order to create the driver")
	public static String CHROMEDRIVER_ADDRESS = "http://localhost:9515";
	
	//@Parameter(key = "app_url", group = "Selenium", description = "Website address in order to reset the driver ()")
	//public static String APP_URL = "http://localhost:3000";

	@Parameter(key = "chrome_binaries", group = "Selenium", description = "Path in the system that points to chrome binaries in order to start the browser in headless mode")
	public static String CHROME_BINARIES = "";
	
	@Parameter(key = "graph_path", group = "Selenium", description = "Path of graph file in dot format")
	public static String GRAPH_PATH = System.getProperty("user.home") + File.separator + "Desktop" + File.separator + "graph.txt";
	
	@Parameter(key = "path_sequence_rate", group = "Selenium", description = "Probability of generating test cases with respect path sequences according to the graph")
	public static double PATH_SEQUENCE_RATE = 1.0;
	
	@Parameter(key = "start_node", group = "Selenium", description = "Start node in the graph")
	public static String START_NODE = "";
	
	@Parameter(key = "test_minimum_size", group = "Selenium", description = "Minimum size of a test case")
	public static int TEST_MIN_SIZE = 5;
	
	@Parameter(key = "insertion_max_size", group = "Selenium", description = "Max number of methods to call in a mutation insertion")
	public static int INSERTION_MAX_SIZE = 5;
	
	@Parameter(key = "path_max_length_subgraph", group = "Selenium", description = "Minimum length of a path during creation of subgraph (see getAllPaths in http://jgrapht.org/javadoc/org/jgrapht/alg/shortestpath/AllDirectedPaths.html)")
	public static int PATH_MAX_LENGTH_SUBGRAPH = 10;
	
	@Parameter(key = "reset_driver", group = "Selenium", description = "Reset or not the driver during evolution and minimization phase of the genetic algorithm")
	public static boolean RESET_DRIVER = true;
	
	@Parameter(key = "custom_mutation_delete", group = "Selenium", description = "Activate or deactivate custom mutation delete based on graph based method sequences")
	public static boolean CUSTOM_MUTATION_DELETE = true;
	
	@Parameter(key = "custom_mutation_insert", group = "Selenium", description = "Activate or deactivate custom mutation insert based on graph based method sequences")
	public static boolean CUSTOM_MUTATION_INSERT = true;
	
	@Parameter(key = "custom_mutation_change", group = "Selenium", description = "Activate or deactivate custom mutation change that changes only primitive values")
	public static boolean CUSTOM_MUTATION_CHANGE = true;
	
	@Parameter(key = "test_case_crossover", group = "Selenium", description = "Activate or deactivate custom crossover at test case level, in addition to crossover at test suite level")
	public static boolean TEST_CASE_CROSSOVER = true;
	
	@Parameter(key = "test_case_crossover_rate", group = "Selenium", description = "Probability of having crossover at test case level, instead of crossover at test suite level")
	public static double TEST_CASE_CROSSOVER_RATE = 0.5;
	
	@Parameter(key = "mutation_change_null_object", group = "Selenium", description = "Activate or deactivate the possibility of generating null object for entity with parameters statements")
	public static boolean MUTATION_CHANGE_NULL_OBJECT = false;
	
	@Parameter(key = "false_goals", group = "Selenium", description = "Activate or deactivate taking only false goals into account in only branch coverage suite fitness")
	public static boolean FALSE_GOALS = true;
	
	@Parameter(key = "select_false_goals", group = "Selenium", description = "Activate or deactivate the selection of only a subset of the false goals during test suite writing")
	public static boolean SELECT_FALSE_GOALS = true;
	
	@Parameter(key = "custom_minimization", group = "Selenium", description = "Activate or deactivate custom minimization desired goals that one would to cover")
	public static boolean CUSTOM_MINIMIZATION = true;
	
	@Parameter(key = "stop_on_true", group = "Selenium", description = "Activate or deactivate search stop after all false branches are covered, independently from the coverage of the true branches")
	public static boolean STOP_ON_TRUE = false;
	
	@Parameter(key = "add_fields_uut", group = "Selenium", description = "Activate or deactivate the possibility of adding public field of the unit under test UUT (dependencies included). We want to take only public methods from the UUT.")
	public static boolean ADD_FIELDS_UUT = true;
	
	@Parameter(key = "reset_app_state", group = "Selenium", description = "Reset application state before each test case execution")
	public static boolean RESET_STATE = false;

	@Parameter(key = "quit_browser_to_reset", group = "Selenium", description = "Reset method implies quitting the browser to clean up the state")
	public static boolean QUIT_BROWSER_TO_RESET = false;
	
	@Parameter(key = "reset_mysql_db", group = "Selenium", description = "Reset db after each test case execution: doesn't apply if RESET_STATE is false")
	public static boolean RESET_MYSQL_DB = false;
	
	@Parameter(key = "reset_mysql_db_name", group = "Selenium", description = "Name of db to reset: doesn't apply if RESET_STATE and RESET_MYSQL_DB are false")
	public static String MYSQL_DB_NAME;
	
	@Parameter(key = "reset_mysql_db_user", group = "Selenium", description = "Username of db to reset: doesn't apply if RESET_STATE and RESET_MYSQL_DB are false")
	public static String MYSQL_DB_USERNAME;
	
	@Parameter(key = "reset_mysql_db_pass", group = "Selenium", description = "Password of db to reset: doesn't apply if RESET_STATE and RESET_MYSQL_DB are false")
	public static String MYSQL_DB_PASSWORD;
	
	@Parameter(key = "reset_mysql_db_tables", group = "Selenium", description = "Db tables to reset separated by ':' : doesn't apply if RESET_STATE and RESET_MYSQL_DB are false")
	public static String[] MYSQL_DB_TABLES = new String[] { "" };
	
	@Parameter(key = "reset_logout", group = "Selenium", description = "Activate or deactivate logout action after execution of each test case: doesn't apply if RESET_STATE is false")
	public static boolean LOGOUT = false;
	
	@Parameter(key = "reset_cookies", group = "Selenium", description = "Clear cookies during logout action after execution of each test case: doesn't apply if RESET_STATE and LOGOUT are false")
	public static boolean RESET_COOKIES = false;

	@Parameter(key = "reset_cookie_names", group = "Selenium", description = "Cookie names to reset separated by ':' : doesn't apply if RESET_STATE, LOGOUT and RESET_COOKIES are false")
	public static String[] COOKIE_NAMES = new String[] { "" };
	
	@Parameter(key = "random_char_number_letters", group = "Selenium", description = "Activate or deactivate random generation of chars which generates only number and letters with equal probability")
	public static boolean RANDOM_CHAR_NUMBER_LETTERS = true;
	
	@Parameter(key = "custom_mosa", group = "Selenium", description = "MOSA changes when MOSA strategy and algorithm are enabled")
	public static boolean CUSTOM_MOSA = false;

	@Parameter(key = "only_false_branches", group = "Selenium", description = "Activate or deactivate the possibility of covering only false branches of the class under test during the evolution")
	public static boolean ONLY_FALSE_BRANCHES = false;

	@Parameter(key = "art_algorithm_num_candidates", group = "Selenium", description = "Adaptive random testing FSCS algorithm, number of candidate solutions to generate at each step of the evolution")
	public static int ART_ALGORITHM_NUM_CANDIDATES = 30;

	@Parameter(key = "art_logging_coverage", group = "Selenium", description = "Adaptive random testing (ART): enable logging of coverage and number of executed test cases at each test case execution.")
	public static boolean ART_LOGGING_COVERAGE = false;

	@Parameter(key = "input_distance", group = "Selenium", description = "Enable or disable input diversity computation in ART.")
	public static boolean INPUT_DISTANCE = false;

	@Parameter(key = "source_code_system_path", group = "Selenium", description = "Source code system path. It is needed for source code parsing in ART.")
	public static String SOURCE_CODE_SYSTEM_PATH = "";

	@Parameter(key = "test_data_interface", group = "Selenium", description = "In ART for computing input diversity: classes in the SOURCE_CODE_SYSTEM_PATH that implement this interface are custom classes")
	public static String TEST_DATA_INTERFACE = "";

	@Parameter(key = "custom_interface_name", group = "Selenium", description = "In ART for computing input diversity: value property of these custom classes that implement this interface has to be considered as it is.")
	public static String CUSTOM_INTERFACE_NAME = "";

	@Parameter(key = "mock_system_millis", group = "Selenium", description = "Enable or disable mocking of System.currentTimeMillis() during <<runtime>> instrumentation.")
	public static boolean MOCK_SYSTEM_MILLIS = true;

	@Parameter(key = "max_sleep_time", group = "Selenium", description = "Max time a thread can sleep during test case execution (if a Thread.sleep() is executed inside a test case) in ms.")
	public static int MAX_SLEEP_TIME = 50;

	@Parameter(key = "debug_test_case", group = "Selenium", description = "Print the test case that is about to be executed, in file debug + application_name.txt in user desktop.")
	public static boolean DEBUG_TEST_CASE = false;

	@Parameter(key = "reset_class_name", group = "Selenium", description = "Qualified name (org.main.Reset -> with the package name) of the class to be instantiated before each test case execution. I assume that the method to be called being reset with one argument that is the app url.")
	public static String RESET_CLASS_NAME = "";

	@Parameter(key = "write_reset_method_in_test_case", group = "Selenium", description = "Enabled or disabled the possibility of adding reset application state at the top of each test case before instantiation of the CUT.")
	public static boolean WRITE_RESET_METHOD_IN_TEST_CASE = false;

	@Parameter(key = "enum_test_data_generation", group = "Selenium", description = "Enable or disable the possibility of generating only values listed in the enum class when a method of a test case accepts an interface as input and the classes that implement this interface are enum.")
	public static boolean ENUM_TEST_DATA_GENERATION = false;

	@Parameter(key = "application_name", group = "Selenium", description = "Name of the project/application you are testing.")
	public static String APPLICATION_NAME = "";

	@Parameter(key = "filter_lines_to_cover", group = "Selenium", description = "Enable or disable the possibility of filtering lines to cover for model transition coverage purposes.")
	public static boolean FILTER_LINES_TO_COVER = false;

	@Parameter(key = "lines_to_cover", group = "Selenium", description = "List of lines to cover obtained using cut-instrumenter project, separeted by ':'. Property used when line coverage criterion is toggled and filter_lines_to_cover = true.")
	public static String[] LINES_TO_COVER = new String[] { "" };

	@Parameter(key = "print_code_after_exception", group = "Selenium", description = "By default evosuite when it writes test cases in the final test suite chops all lines of code in a test case after an exception. When this property is true this will not happen.")
	public static boolean PRINT_CODE_AFTER_EXCEPTION = false;

	@Parameter(key = "add_sleep_statements_in_test_case", group = "Selenium", description = "Enable re-execution of a test case with a higher sleep timeout if the test case fails for a selenium exception.")
	public static boolean ADD_SLEEP_STATEMENTS_IN_TEST_CASE = false;

	@Parameter(key = "max_attempts_execution_test_case", group = "Selenium", description = "Max number of times a test case can be re-executed if it failed because of a selenium exception before giving up.")
	public static int MAX_ATTEMPTS_EXECUTION_TEST_CASE = 0;

	@Parameter(key = "cut_exceptions", group = "Selenium", description = "If CUT uses custom exceptions instead of return statements when preconditions are not satisfied.")
	public static boolean CUT_EXCEPTIONS = false;

	@Parameter(key = "custom_exceptions", group = "Selenium", description = "Custom exceptions used by the CUT.")
	public static String[] CUSTOM_EXCEPTIONS = new String[] { "" };

	@Parameter(key = "queue_art", group = "Selenium", description = "Use a queue instead of a list to store already executed test cases in ART.")
	public static boolean QUEUE_ART = false;
