...
public class IndividualTestStrategy extends TestGenerationStrategy {
	...
	public TestSuiteChromosome generateTests() {
		...
		logger.debug("@@@@ factory chromosome type: {}", factory.getChromosomeFactory().getClass().getName());
        ...
        if(Properties.SELENIUM){
        	/*TestFitnessFunction goalToCover = null;
         	for(TestFitnessFunction goal : goals){
         		if(goal.toString().contains("IFEQ") && goal.toString().contains("false")){
         			goalToCover = goal;
         		}
         	}
         	if(goalToCover != null){
         		goals.clear();
         		goals.add(goalToCover);
         	}
         	else
         	{
         		logger.error(":::::no goal meets the condition - generateTests function");
         	}*/
         	
         	for(int i = 0; i < goals.size(); i++){
            	logger.debug("@@@@goal to cover {}: {}",i,goals.get(i).toString());
            }
        }
		...
		logger.debug("@@@@TestSuiteChromosome before evolution: " + suite.getClass().getName());
		...
		while (current_budget < total_budget && covered_goals < total_goals
			...
			for (TestFitnessFunction fitnessFunction : goals) {
				...
				if (ga.getBestIndividual().getFitness() == 0.0) {
					...
					{
						logger.debug("@@@@Calculate and keep track of overall fitness");
					...
					if (Properties.SKIP_COVERED) {
						...
						logger.debug("@@@@Covered goals after getAdditionallyCoveredGoals: " + covered_goals);
					}
