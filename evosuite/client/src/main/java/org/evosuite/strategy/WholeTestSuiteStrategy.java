...
public class WholeTestSuiteStrategy extends TestGenerationStrategy {
	...
	public TestSuiteChromosome generateTests() {
		...
		/*for(int i = 0; i < fitnessFunctions.size(); i++){
			OnlyBranchCoverageSuiteFitness o = (OnlyBranchCoverageSuiteFitness) fitnessFunctions.get(i);
			LoggingUtils.getEvoLogger().debug("$$$$fitnessFunction {}: {}",i,fitnessFunctions.get(i).toString());
        }*/
...
//			algorithm.addFitnessFunction(f);
		...
		Set<StoppingCondition> stoppingConditions = algorithm.getStoppingConditions();
		for(StoppingCondition stoppingCondition: stoppingConditions){
			LoggingUtils.getEvoLogger().debug("@@@@stoppingCondition: " + stoppingCondition.getClass().getName());
		}
