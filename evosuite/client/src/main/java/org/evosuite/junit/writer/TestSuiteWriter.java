...
import org.evosuite.coverage.branch.OnlyFalseBranchCoverageFactory;
...
public class TestSuiteWriter implements Opcodes {
    ...
    public List<File> writeTestSuite(String name, String directory, List<ExecutionResult> cachedResults) throws IllegalArgumentException {
        ...
        if(Properties.CUT_EXCEPTIONS){
            // do not execute test suite again since it was executed in the minimization phase
            if(!Properties.MINIMIZE) throw new IllegalArgumentException("writeTestSuite: when 'cut_exceptions' = true also 'minimize' must be true");
            /* in this case the test 'result.test == test' always fails
            since the test executed is not the original one but a test case with sleep statements inserted
            therefore I remove this check and I take the cachedResults checking that each result is not null
            * */
//            for (int i = 0; i < testCases.size(); i++) {
//                TestCase test = testCases.get(i);
//                logger.info("Cached results size: " + cachedResults.size());
...
//            }
            ...
            cachedResults.forEach(cachedResult -> {
                if(cachedResult != null) results.add(cachedResult);
                else throw new IllegalStateException("writeTestSuite - cachedResult must not be null");
            });

            if(results.size() == 0) throw new IllegalStateException("writeTestSuite: execution result list must not be empty");
        }else{
            for (int i = 0; i < testCases.size(); i++) {
                TestCase test = testCases.get(i);
                boolean added = false;
    ...
    private String getUnitTestsAllInSameFile(String name, List<ExecutionResult> results) {
        ...
        List<OnlyBranchCoverageTestFitness> falseGoals = new OnlyBranchCoverageFactory().getInteresingCoverageGoals();
        boolean isInterestingGoalCovered = false;
        ...
        if(testCases.isEmpty()) {
        	...
        	for (int i = 0; i < testCases.size(); i++) {
        		if(Properties.SELECT_FALSE_GOALS){
        			TestCase testCase = testCases.get(i);
                    Set<TestFitnessFunction> coveredGoals = testCase.getCoveredGoals();
                    for(TestFitnessFunction coveredGoal: coveredGoals){
                    	if(coveredGoal instanceof OnlyBranchCoverageTestFitness){
                    		OnlyBranchCoverageTestFitness goal = (OnlyBranchCoverageTestFitness) coveredGoal;
                    		logger.debug("$$$$covered goal test case " + i + ": " + goal.toString());
                    		if(falseGoals.contains(goal)){
                    			logger.debug("$$$$interesting");
                    			isInterestingGoalCovered = true;
                    		}
                    	}
                    }
                    if(isInterestingGoalCovered){
                    	builder.append(testToString(i, i, results.get(i)));
                    	isInterestingGoalCovered = false;
                    }
        		}
        		else{
        			builder.append(testToString(i, i, results.get(i)));
    ...
    protected String testToString(int number, int id, ExecutionResult result) {
        ...
        if(!Properties.PRINT_CODE_AFTER_EXCEPTION){
        ...
        if(Properties.WRITE_RESET_METHOD_IN_TEST_CASE){
            builder.append(CODE_SPACE);
            //builder.append(Properties.RESET_CLASS_NAME + ".reset(\"" + Properties.APP_URL + "\");");
            builder.append(Properties.RESET_CLASS_NAME + ".reset();");
            builder.append(NEWLINE);
        }
    ...
    private void writeCoveredGoalsFile() {
        if (Properties.WRITE_COVERED_GOALS_FILE) {
            ...
            List<TestFitnessFunction> uniqueCoveredGoals = new ArrayList<>();
            for (int i = 0; i < testCases.size(); i++) {
                TestCase test = testCases.get(i);
                ...
                for (TestFitnessFunction goal : coveredGoals) {
                    ...
                    if(!uniqueCoveredGoals.contains(goal)) {
                        logger.info("Covered goal___" + goal.toString() + NEWLINE);
                        uniqueCoveredGoals.add(goal);
                    }
