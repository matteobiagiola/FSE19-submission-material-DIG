		...
		try {
			return super.exceptionHandler(new Executer() {
				...
				public void execute() throws InvocationTargetException,
					...
					//logger.debug(".........Constructor scope: " + scope.toString());
					
					for (int i = 0; i < parameters.size(); i++) {
						VariableReference parameterVar = parameters.get(i);
						try {
							/*WebDriver driver = Scope.getDriver().getWebDriver();
							WebElement element = driver.findElement(By.id("second-category"));
							element.click();*/
					...
					//logger.debug("--");
					//logger.debug("--");
					//logger.debug("Driver state: {}", Driver.driver);
					Object ret = constructor.getConstructor().newInstance(inputs);
					//logger.debug("+++++++++ret: ret {} - retClass {}",ret,ret.getClass());
