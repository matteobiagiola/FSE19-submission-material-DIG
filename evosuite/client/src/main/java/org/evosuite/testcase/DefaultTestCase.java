...
public class DefaultTestCase implements TestCase, Serializable {
	...
	private boolean hasMinimizationFailed = false;

	private int id;
	
	//string mapping for the statements in this test case: SELENIUM_WHOLE for path sequences inside a graph
	...
	public DefaultTestCase clone() {
		...
		*/
		...
		// isMinimized property is not cloned, it is set every time it is needed
	...
	/*public Statement returnStatement(Class<?> statementClass){
		for(Statement s: statements){
	...
	@Override
	public boolean hasMinimizationFailed(){
		return this.hasMinimizationFailed;
	}

	@Override
	public void setMinimizedFailed(boolean isMinimized){
		this.hasMinimizationFailed = isMinimized;
	}
