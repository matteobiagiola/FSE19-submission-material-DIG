...
public class LineCoverageFactory extends
	...
	public List<LineCoverageTestFitness> getCoverageGoals() {
		return getCoverageGoals(Properties.FILTER_LINES_TO_COVER);
	}

	private List<LineCoverageTestFitness> getCoverageGoals(boolean filterLinesToCover) {

		List<LineCoverageTestFitness> goals = new ArrayList<LineCoverageTestFitness>();
		long start = System.currentTimeMillis();

		if(!filterLinesToCover) {
			...
			List<String> linesToCoverString = Arrays.asList(Properties.LINES_TO_COVER);
			List<Integer> linesToCover = linesToCoverString.stream().map(Integer::valueOf).collect(Collectors.toList());
			if(linesToCover.size() == 0)
