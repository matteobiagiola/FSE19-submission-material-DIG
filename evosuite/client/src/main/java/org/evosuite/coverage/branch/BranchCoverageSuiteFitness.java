...
public class BranchCoverageSuiteFitness extends TestSuiteFitnessFunction {
	...
	protected void determineCoverageGoals() {
		...
		BranchCoverageTestFitness firstGoal = null;
		BranchCoverageTestFitness lastGoal = null;
		for (BranchCoverageTestFitness goal : goals) 
		{
			...
			{
				firstGoal = goal;
				logger.debug("^^^firstGoal: {}",firstGoal.toString());
			}
			if(count == goals.size() - 1)
			{
				lastGoal = goal;
				logger.debug("^^^lastGoal: {}",lastGoal.toString());
			}
			...
			{
				if(goal.equals(firstGoal) || goal.equals(lastGoal))
				{
