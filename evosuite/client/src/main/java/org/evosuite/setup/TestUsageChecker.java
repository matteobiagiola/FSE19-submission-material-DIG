...
public class TestUsageChecker {
    ...
    public static boolean canUse(Class<?> c) {
    	...
    	//logger.debug(">>>>>class canUse method: {}",c.getName());
    	/*if(Properties.SELENIUM_WHOLE){
    		if(c.getName().contains("firefox.Driver") 
        			|| c.getName().contains("chrome.Driver") 
        			|| c.getName().contains("phantomjs.Driver") 
        			|| c.getName().contains("examples.Driver")
        			|| c.getName().contains("org.openqa.selenium.WebDriver"))
        	{
        		logger.debug(">>>>>es does not play with: {}",c.getName());
        		return false;
        	}
    	}*/
    ...
    public static boolean canUse(Field f, Class<?> ownerClass) {
    	...
    	//logger.debug(">>>>>ownerClass canUse method and field: {}, {}",ownerClass.getName(),f.getName());
    	/*if(ownerClass.getName().contains("firefox.Driver") 
    			|| ownerClass.getName().contains("chrome.Driver") 
    			|| ownerClass.getName().contains("phantomjs.Driver")
    			|| ownerClass.getName().contains("examples.Driver"))
    	{
    		//logger.debug(">>>>>do not add getDriver method: {}",ownerClass.getName());
    		return false;
    	}*/
        // TODO we could enable some methods from Object, like getClass
    ...
    public static boolean canUse(Method m, Class<?> ownerClass) {
        ...
        /*if(Properties.SELENIUM_WHOLE){
        	logger.debug("####canUse method: " + m.getName() + " declaring class: " + m.getDeclaringClass().getName() + " cut: " + Properties.TARGET_CLASS + " prefix: " + Properties.TARGET_CLASS_PREFIX);
        	if(m.getDeclaringClass().getName().equals(Properties.TARGET_CLASS)){
        		logger.debug("####using method: " + m);
        		return true;
        	}else{
        		logger.debug("####c method: " + m);
        		return false;
        	}
        }*/
