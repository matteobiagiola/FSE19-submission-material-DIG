...
usage() {
  echo "Run phoenix trello application testing"
  echo "Valid options:"
  ...
  echo " -p STRING [phoenix], project name; name of the docker container, session file name, ES logs name"
  echo " -d NUMBER [5432], port of postgres db for remote connection"
...
if [ "$STRATEGY" == "mosa" ] ; then
  if [ "$APOGEN" == "true" ] ; then
    ./run.sh -class main.ClassUnderTestApogen -generateMOSuite -Dtest_dir=$TEST_DIR \
        -Dgraph_path=workspace/graphs/phoenix-apogen.txt -Dstart_node=IndexPage \
        -Dlines_to_cover=27:41:56:70:89:103:121:135:150:168:183:205:226:245:263:277:291:311:314:329:355:358:377:391 \
  ...
  ./run.sh -class main.ClassUnderTest -Dtest_dir=$TEST_DIR -Dreport_dir=$TEST_DIR/evosuite-report \
   -Dsearch_budget=$SEARCH_BUDGET -criterion line > ~/Desktop/logs$SUFFIX.txt 2> ~/Desktop/errors$SUFFIX.txt
elif [ "$STRATEGY" == "adaptiveSequence" ] ; then
  if [ "$APOGEN" == "true" ] ; then
  ...
  ./run.sh -class main.ClassUnderTest -generateCustomRandom -Dtest_dir=$TEST_DIR \
     -Dreport_dir=$TEST_DIR/evosuite-report -Dsearch_budget=$SEARCH_BUDGET -criterion line > ~/Desktop/logs$SUFFIX.txt 2> ~/Desktop/errors$SUFFIX.txt
fi
sleep 2
