...
usage() {
  echo "Run petclinic application testing"
  echo "Valid options:"
  ...
  echo " -p STRING [petclinic], project name; name of the docker container, session file name, ES logs name"
...
if [ "$STRATEGY" == "mosa" ] ; then
  if [ "$APOGEN" == "true" ] ; then
    ./run.sh -class main.ClassUnderTestApogen -generateMOSuite -Dtest_dir=$TEST_DIR \
        -Dgraph_path=workspace/graphs/petclinic-apogen.txt -Dstart_node=IndexPage \
        -Dlines_to_cover=27:42:57:74:95:114:128:145:166:185:200:218:232:246:260:279:297:311:325:339 \
