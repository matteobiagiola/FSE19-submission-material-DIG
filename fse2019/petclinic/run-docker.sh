...
if [[ $PRODUCTION == "yes"  ]]; then
	...
	if [[ $CODE_COVERAGE_INSTRUMENTATION == "yes" ]]; then
		...
		--entrypoint ./run-code-instrumentation.sh dockercontainervm/petclinic:latest bash
    else
        docker run -it --workdir=/home/spring-petclinic-angularjs --name=$CONTAINER_NAME \
		--expose 8080 --expose 3306 -p $PORT_APP:8080 -p $PORT_DB:3306 -d \
		--env PATH=/root/workspace/maven/apache-maven-3.5.4/bin:/root/workspace/java/jdk1.8.0_181/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin \
	...
	if [[ $CONTAINER_NAME != "default" ]]; then
		docker run -it --workdir=/home/spring-petclinic-angularjs --name=$CONTAINER_NAME --expose 8080 --expose 3306 \
		    -p $PORT_APP:8080 -p $PORT_DB:3306 dockercontainervm/petclinic:latest bash
	else
		docker run -it --workdir=/home/spring-petclinic-angularjs --expose 8080 --expose 3306 -p $PORT_APP:8080 \
		    -p $PORT_DB:3306 dockercontainervm/petclinic:latest bash
	fi
fi
