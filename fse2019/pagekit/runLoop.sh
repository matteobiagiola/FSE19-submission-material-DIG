...
usage() {
  echo "Run pagekit application testing"
  echo "Valid options:"
  ...
  echo " -p STRING [pagekit], project name; name of the docker container, session file name, ES logs name"
...
while getopts 'hi:s:c:p:d:a:b:r:t:g:' arg
    do
        case ${arg} in
	  h)
		    usage
...
if [ "$STRATEGY" == "mosa" ] ; then
  if [ "$APOGEN" == "true" ] ; then
    ./run.sh -class main.ClassUnderTestApogen -generateMOSuite -Dtest_dir=$TEST_DIR \
        -Dgraph_path=workspace/graphs/pagekit-apogen.txt -Dstart_node=DashboardPage \
        -Dlines_to_cover=28:42:56:70:88:113:131:152:177:195:217:220:239:253:267:281:295:309:323:337:351:369:389:407:426:449:468:482:496:514:528:542:556:569:583:600:618:633:648:665:680:695 \
    ...
    ./run.sh -class main.ClassUnderTest -Dtest_dir=$TEST_DIR -Dreport_dir=$TEST_DIR/evosuite-report -Dsearch_budget=$SEARCH_BUDGET -criterion line > ~/Desktop/logs$SUFFIX.txt 2> ~/Desktop/errors$SUFFIX.txt
elif [ "$STRATEGY" == "adaptiveSequence" ] ; then
    if [ "$APOGEN" == "true" ] ; then
    ...
    if [ "$APOGEN" == "true" ] ; then
     ...
     ./run.sh -class main.ClassUnderTest -generateCustomRandom -Dtest_dir=$TEST_DIR -Dreport_dir=$TEST_DIR/evosuite-report -Dsearch_budget=$SEARCH_BUDGET -criterion line > ~/Desktop/logs$SUFFIX.txt 2> ~/Desktop/errors$SUFFIX.txt
     fi
sleep 2
