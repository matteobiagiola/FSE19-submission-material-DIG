...
usage() {
  echo "Run splittypie application testing"
  echo "Valid options:"
  ...
  echo " -p STRING [splittypie], project name; name of the docker container, session file name, ES logs name"
  echo " -a NUMBER [4200], port of the application server for remote connection"
...
# sleep 60 # let the application server start
# echo "Application server should be ready"

# while [ $COUNTER -lt $ITERATIONS ]
# do
#     SUFFIX=$PROJECT_NAME"_"$COUNTER
#     TEST_DIR=$HOME/Desktop/test$SUFFIX
#     mkdir $HOME/Desktop/test$SUFFIX
# 	if [ "$STRATEGY" == "mosa" ] ; then
# 		./run.sh -class main.ClassUnderTest -generateMOSuite -Dtest_dir=$TEST_DIR -Dreport_dir=$TEST_DIR -Dcovered_goals_file=$TEST_DIR -Dsearch_budget=$SEARCH_BUDGET -criterion onlybranch > ~/Desktop/logs$SUFFIX.txt 2> ~/Desktop/errors$SUFFIX.txt
# 	elif [ "$STRATEGY" == "whole" ] ; then
# 	    ./run.sh -class main.ClassUnderTest -criterion onlybranch -Dtest_dir=$TEST_DIR -Dreport_dir=$TEST_DIR -Dcovered_goals_file=$TEST_DIR -Dsearch_budget=$SEARCH_BUDGET > ~/Desktop/logs$SUFFIX.txt 2> ~/Desktop/errors$SUFFIX.txt
# 	elif [ "$STRATEGY" == "adaptiveSequence" ] ; then
#        ./run.sh -class main.ClassUnderTest -generateAdaptiveRandom -Dtest_dir=$TEST_DIR -Dreport_dir=$TEST_DIR -Dcovered_goals_file=$TEST_DIR -Dsearch_budget=$SEARCH_BUDGET -criterion onlybranch -Dinput_distance=false > ~/Desktop/logs$SUFFIX.txt 2> ~/Desktop/errors$SUFFIX.txt
#     elif [ "$STRATEGY" == "adaptiveComplete" ] ; then
#        ./run.sh -class main.ClassUnderTest -generateAdaptiveRandom -Dtest_dir=$TEST_DIR -Dreport_dir=$TEST_DIR -Dcovered_goals_file=$TEST_DIR -Dsearch_budget=$SEARCH_BUDGET -criterion onlybranch -Dinput_distance=true > ~/Desktop/logs$SUFFIX.txt 2> ~/Desktop/errors$SUFFIX.txt
# 	elif [ "$STRATEGY" == "random" ] ; then
#        ./run.sh -class main.ClassUnderTest -generateCustomRandom -Dtest_dir=$TEST_DIR -Dreport_dir=$TEST_DIR -Dcovered_goals_file=$TEST_DIR/covered.goals -Dsearch_budget=$SEARCH_BUDGET -criterion onlybranch > ~/Desktop/logs$SUFFIX.txt 2> ~/Desktop/errors$SUFFIX.txt
#        fi
# 	sleep 2
...
if [ "$STRATEGY" == "mosa" ] ; then
  if [ "$APOGEN" == "true" ]; then
    ./run.sh -class main.ClassUnderTestApogen -generateMOSuite -Dtest_dir=$TEST_DIR \
        -Dgraph_path=workspace/graphs/splittypie-apogen.txt -Dstart_node=IndexPage \
        -Dlines_to_cover=30:45:59:77:91:105:119:138:156:171:195:215:231:247:262:278:291:309:329:347:365:385:404:423:442:460:475:496:516:532:557 \
